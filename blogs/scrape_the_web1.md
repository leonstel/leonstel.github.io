...
So we are off on the right foot, to move on this is our next page with useful information. As you can see it has redirected
...
pinpoint which elements you need and find characteristics of the html tag to identify that specific part(s) of the
...
against within current tournament. We are after the matches' scores and opponent players. 
...
into the database. The `extractMatches()` has saved the matches to the `matches_df` global variable.
...
def processMatches(tournament_id):
    ...
    for index, match_row in globals.matches_df.iterrows():
        ...
        # get the player ids from the player_id_cache and use them for saving the match to database
        # the match needs the player ids for its relation with the players table
...
I believe that databases are really good for capturing relationships in comparison with excel and csv. On top of that databases
...
Example query which does some basic selecting, but you could go crazy with the captured data. 
